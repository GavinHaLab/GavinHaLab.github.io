---
title: 'Homework 7: SNV Genotyping'
subtitle: 'GENOME 541: Cancer Genomics Module (Spring 2023s)'
...
The input data is a text file `Homework7_alleleCounts.txt` that contains a set of 1000 loci with reference and non-reference allele counts. I provided the outputs for each function so that you can use it to check your code.  
...
Code template files `Homework7_SNVGenotyping_template.Rmd` and `Homework7_SNVGenotyping_template.ipynb` are provided for you to write your code and to generate outputs.

Please submit the following files:

  1. The code as an R Markdown (`Homework7_code_FirstName-LastName.Rmd`) or Python Jupyter Notebook (`Homework7_code_FirstName-LastName.ipynb`) file. 
  2. The exported PDF (`Homework7_code_FirstName-LastName.pdf`) of the code and output. 
  3. The final output file of the results in Section 3.1 (`Homework7_mutationCalls_FirstName-LastName.txt`)
...
```{r countfile}
counts <- read.delim("Homework7_alleleCounts.txt", as.is = TRUE)
counts[1:2, ]
x <- counts$refCount
N <- counts$depth
```
...
Refer to Lecture 2 Slide 12 for the EM algorithm. Run the EM algorithm until convergence, compute.log.posterior. Convergence is acheived when the log posterior changes by less than a user-defined valued, $\epsilon = 10^{-2}$.
...
Append this to the original table. Save the results to a file, named `Homework7_mutationCalls_FirstName-LastName.txt`, with the same columns as the input file (`counts`) and then appending an additional column with the mutation status (`NotSNV` or `SNV`)
...
Plot the probability mass function (pmf) for each binomial distribution in the 3-component mixture using the converged parameters $\hat{\mu}_{1:K}$. Use $N=40$ depth and a range of values $x=\{0,\ldots,40\}$ reference read counts. The plot should look similar to the 3 colored binomial pmfs in Lecture 2, slide 25.
