---
title: 'Homework 8: Continuous HMM for profiling copy number alterations'
subtitle: 'GENOME 541: Cancer Genomics Module (Spring 2023)'
...
Please submit the following files:
  ...
  1. The code as an R Markdown (`Homework8_code_FirstName-LastName.Rmd`) or Python Jupyter Notebook (`Homework8_code_FirstName-LastName.ipynb`) file. 
  2. The exported PDF (`Homework8_code_FirstName-LastName.pdf`) of the code and output. 
...
Load the input data from `Homework8_log2ratios_chr1.txt`. This file contains $T$ bins (1Mb) where each bin $t$ has a read count (`readCount`) and a corrected $\log_2$ ratio (`log2Ratio`). The column `log2Ratio` is the data, $x_{1:T}$, that we will use for this assignment. 
```{r countfile, echo=TRUE}
LogRatios <- read.delim("Homework8_log2ratios_chr1.txt", as.is = TRUE)
